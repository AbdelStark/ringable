...
export default function ProposalDetailsPage() {
  ...
  const { getProposalById, addVote, getResults, closeProposal } = useProposalsStore();
  ...
  React.useEffect(() => {
    if (!proposal || !ring || !keyPair) {
      setEligibility({ eligible: false, reason: !keyPair ? "Generate a keypair first." : !proposal ? "Proposal not found." : "Voting ring not found." });
      return;
    }

    if (!ring.memberPublicKeys.includes(keyPair.publicKeyHex)) {
      setEligibility({ eligible: false, reason: "Your key is not in the voting ring for this proposal." });
      return;
    }
    ...
    const checkVote = async () => {
      ...
      if (foundVote) {
        ...
        setEligibility({ eligible: false, reason: "Voting for this proposal is closed." });
      } else {
  ...
  const handleVote = async (optionId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setEligibility({ eligible: false, reason: "Vote successfully recorded." });
  ...
  const handleCloseProposal = () => {
    if (!proposal) return;
    if (window.confirm("Are you sure you want to close voting for this proposal? This cannot be undone.")) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-xl font-bold uppercase tracking-wider break-all mr-4">{proposal.title}</h2>
        <Link href="/" className="text-sm text-pixel-accent hover:underline shrink-0">
          &lt;- Back to Home
        </Link>
      </div>
      <p className={`text-xs font-bold px-2 py-0.5 border-2 inline-block ${proposal.status === 'open' ? 'border-green-500 text-green-600 bg-green-100' : 'border-pixel-disabled text-pixel-disabled bg-gray-200'}`}>
      ...
      {proposal.status === 'open' && (
         <Card title="Admin Actions">
            <Button variant="secondary" className="text-pixel-accent" onClick={handleCloseProposal}>
              Close Voting
            </Button>
            <p className="text-xs text-gray-500 mt-2">Closing voting prevents further votes and finalizes results.</p>
         </Card>
      )}
    </div>
  );
