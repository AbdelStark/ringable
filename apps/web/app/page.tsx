      ...
      {proposals.length === 0 ? (
        ...
        <div className="space-y-4">
          {proposals.map((proposal) => (
            <Link key={proposal.id} href={`/proposals/${proposal.id}`} className="block hover:no-underline">
              <Card className="hover:border-pixel-accent transition-colors duration-150 ease-in-out">
                <div className="flex justify-between items-center">
                  <h3 className="text-base font-normal uppercase tracking-wider break-all mr-4">{proposal.title}</h3>
                  <span className={`text-xs font-bold px-2 py-0.5 border-2 ${proposal.status === 'open' ? 'border-green-500 text-green-600 bg-green-100' : 'border-pixel-disabled text-pixel-disabled bg-gray-200'}`}>
                    {proposal.status.toUpperCase()}
                  </span>
                </div>
                <p className="text-xs text-gray-600 mt-1">Options: {proposal.options.length} | Votes: {proposal.votes.length}</p>
              </Card>
            </Link>
          ))}
        </div>
