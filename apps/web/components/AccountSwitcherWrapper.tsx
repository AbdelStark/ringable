...
export function AccountSwitcherWrapper() {
  const { accounts, activeAccountId, setActiveAccount, generateAccount } = useAccountsStore();
    ...
    const migrateKeyPair = async () => {
      ...
      if (keyPair && accounts.length === 0) {
        try {
          ...
          if (id) {
            setActiveAccount(id);
            addToast("Your account has been migrated to the new accounts system.", "info");
          }
        } catch (error) {
